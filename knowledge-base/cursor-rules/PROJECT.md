...
| `project_posts` | Витрина проектов (модерация; не путать с устаревшим названием community_posts) |
...
| Эндпоинт | Назначение |
|----------|-----------|
| `POST /api/ai-tutor` | AI-тьютор: отвечает на вопросы по теме урока (не `/api/chat`) |
| `POST /api/check-homework` | Проверка домашнего задания ученика |
| `GET /api/ai-usage` | Квота / остаток лимитов AI на сегодня |
| `GET /api/health` | Health + флаг AI |
