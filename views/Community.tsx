            ...
            <div className="grid grid-cols-1 gap-3 md:grid-cols-2 xl:grid-cols-3">
              {filteredStudents.map((s, i) => {
                ...
                return (
                  ...
                  >
                    <button type="button" onClick={() => navigate(`/users/${s.id}`)} className="flex min-w-0 flex-1 items-center gap-4 text-left">
                      <AvatarImage src={avatarUrl} name={s.name || ''} alt="" className="h-14 w-14 shrink-0 rounded-2xl border border-white/10 object-cover" />
                    ...
                    <div className="flex shrink-0 flex-wrap gap-2 md:flex-col md:items-stretch">
                      ...
                      {canFriend && !isFriend && !hasOutgoing && (
                        <button
                          ...
                          className="inline-flex items-center justify-center gap-1.5 rounded-xl bg-kiddy-cherry/90 px-4 py-2 text-xs font-bold uppercase tracking-wider text-white hover:bg-kiddy-cherry disabled:opacity-50"
                        >
                          {busyId === s.id ? <Loader2 size={14} className="animate-spin" /> : <UserPlus size={14} />}
                          В друзья
                        </button>
                      )}
                      {canFriend && hasOutgoing && (
                        <span className="inline-flex items-center justify-center gap-1 rounded-xl border border-amber-500/25 bg-amber-500/10 px-3 py-2 text-[10px] font-bold uppercase tracking-wider text-amber-200">
                          <Clock size={12} /> Ждём
                        </span>
                      )}
                      {isFriend && (
                        <span className="inline-flex items-center justify-center gap-1 rounded-xl border border-emerald-500/25 bg-emerald-500/10 px-3 py-2 text-[10px] font-bold uppercase tracking-wider text-emerald-200">
                          <UserCheck size={12} /> Друзья
                        </span>
                      )}
                    </div>
                  </Card>
